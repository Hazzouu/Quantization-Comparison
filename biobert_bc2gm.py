...
def eval_loop(batch_size, model, layer5_acts, layer11_acts):
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            ...
            id2label = {int(k): str(v) for k, v in model.config.id2label.items()}
            # Define dataset's specific dictionary based on its label_list
            dataset_id2label = {i: label for i, label in enumerate(label_list)}
            
            # Identify the correct B and I tags dynamically for this dataset
            target_b = dataset_id2label.get(1, "B-GENE")
            target_i = dataset_id2label.get(2, "I-GENE")
            ...
            for i in range(labels.shape[0]):
                ...
                # 3. The String Normalizer: Force model strings to match the dataset format
                preds = []
                for rp in raw_preds:
                    rp_upper = rp.upper()
                    if rp_upper.startswith("B"): 
                        preds.append(target_b)
                    elif rp_upper.startswith("I"): 
                        preds.append(target_i)
                    else: 
                        preds.append("O")
